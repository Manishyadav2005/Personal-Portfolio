...
You can execute the `CreateContactRequest` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
createContactRequest(vars: CreateContactRequestVariables): MutationPromise<CreateContactRequestData, CreateContactRequestVariables>;
...
```typescript
createContactRequest(dc: DataConnect, vars: CreateContactRequestVariables): MutationPromise<CreateContactRequestData, CreateContactRequestVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the createContactRequestRef:
```typescript
const name = createContactRequestRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `CreateContactRequest` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `CreateContactRequestData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface CreateContactRequestData {
  contactRequest_insert: ContactRequest_Key;
}
```
### Using `CreateContactRequest`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, createContactRequest, CreateContactRequestVariables } from '@dataconnect/generated';

// The `CreateContactRequest` mutation requires an argument of type `CreateContactRequestVariables`:
...
// Call the `createContactRequest()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await createContactRequest(createContactRequestVars);
// Variables can be defined inline as well.
const { data } = await createContactRequest({ userId: ..., message: ..., senderEmail: ..., senderName: ..., subject: ..., });
...
// Or, you can use the `Promise` API.
createContactRequest(createContactRequestVars).then((response) => {
  const data = response.data;
  console.log(data.contactRequest_insert);
});
```

### Using `CreateContactRequest`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, createContactRequestRef, CreateContactRequestVariables } from '@dataconnect/generated';
...
// Call the `createContactRequestRef()` function to get a reference to the mutation.
const ref = createContactRequestRef(createContactRequestVars);
// Variables can be defined inline as well.
const ref = createContactRequestRef({ userId: ..., message: ..., senderEmail: ..., senderName: ..., subject: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = createContactRequestRef(dataConnect, createContactRequestVars);
