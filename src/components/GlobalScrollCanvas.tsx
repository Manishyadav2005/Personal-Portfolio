...
const GlobalScrollCanvas: React.FC = () => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const imagesRef = useRef<HTMLImageElement[]>([]);
  const targetFrameRef = useRef<number>(0);
  const currentFrameRef = useRef<number>(0);
  const lastDrawnValueRef = useRef<number>(-1);
  const hasDrawnFirstFrameRef = useRef<boolean>(false);
  ...
  const drawFrame = (currentProgress: number): boolean => {
  ...
  const updateCanvasSize = () => {
    ...
    if (drawFrame(currentFrameRef.current)) {
      ...
      lastDrawnValueRef.current = currentFrameRef.current;
    }
  };
  ...
  useEffect(() => {
    ...
    // 1. Preload all 240 frames & draw frame 0 the instant it becomes available
    ...
    for (let i = 0; i < TOTAL_FRAMES; i++) {
      ...
      if (img.complete && img.naturalWidth > 0 && !hasDrawnFirstFrameRef.current) {
        if (drawFrame(currentFrameRef.current)) {
          hasDrawnFirstFrameRef.current = true;
          lastDrawnValueRef.current = currentFrameRef.current;
        }
    ...
    // 2. Track global page scroll from header (top) to footer (bottom)
    const handleScroll = () => {
    ...
    const tick = () => {
      ...
      const needsInitialDraw = !hasDrawnFirstFrameRef.current;
      const hasScrolledDiff = Math.abs(currentVal - lastDrawnValueRef.current) > 0.005;

      if (needsInitialDraw || hasScrolledDiff) {
        if (drawFrame(currentVal)) {
          hasDrawnFirstFrameRef.current = true;
...
export default GlobalScrollCanvas;
